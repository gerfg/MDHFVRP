...
MyCutCallback::MyCutCallback(IloEnv env, const IloArray <IloArray < IloArray< IloBoolVarArray > > >& x_ref, Data* data) : IloCplex::UserCutCallbackI(env), x(x_ref), 
            ...
            if(!data->A_[i][j]){ //If arc is valid
                for(int k = 0; k < data->v; ++k){
                    for(int d = 0; d < data->m; ++d){
                        solX[i][j] += getValue(x[i][j][k][d]);
                        solX[j][i] += getValue(x[i][j][k][d]);
                    }
                }
            }if(!data->A_[j][i]){ //If arc is valid
                for(int k = 0; k < data->v; ++k){
                    for(int d = 0; d < data->m; ++d){
                        solX[i][j] += getValue(x[j][i][k][d]);
                        solX[j][i] += getValue(x[j][i][k][d]);
                    }
                }
            }
        }
